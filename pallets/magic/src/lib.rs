    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn change_controller(
            ...
            let current_controller =
                <ControllerAccountOf<T>>::get(&magic_account).ok_or(Error::<T>::NotExists)?;

            let mut meta =
                <StableAccountOf<T>>::get(&current_controller).ok_or(Error::<T>::NotExists)?;
            ...
            <StableAccountOf<T>>::remove(&current_controller);
            <StableAccountOf<T>>::insert(&meta.controller_account, &meta);

            <ControllerAccountOf<T>>::mutate(&meta.magic_account, |maybe| {
                *maybe = Some(new_controller)
            });

            <Controller<T>>::insert(&meta.stash_account, &meta.controller_account);
        ...
        pub fn codo(origin: OriginFor<T>, call: Box<<T as Config>::Call>) -> DispatchResult {
            let controller_account = ensure_signed(origin)?;
            let meta =
                <StableAccountOf<T>>::get(&controller_account).ok_or(Error::<T>::NotExists)?;
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            ...
            for i in 0..length {
                ...
                <StableAccountOf<T>>::insert(&meta.controller_account, &meta);
                <ControllerAccountOf<T>>::insert(&meta.magic_account, &meta.controller_account);
                <Controller<T>>::insert(&meta.stash_account, &meta.controller_account);
            }
        }
    }
...
impl<T: Config> parami_traits::Accounts for Pallet<T> {
    ...
    fn fee_account(account: &AccountOf<T>) -> AccountOf<T> {
        if let Some(account) = <ControllerAccountOf<T>>::get(account) {
            account
        } else if let Some(account) = <Controller<T>>::get(account) {
            account
        } else {
            // <StableAccountOf<T>>::contains_key(account)
